...
import {
  Package, MapPin, Search, Filter, X, ChevronLeft,
  MessageSquare, LogIn, Loader2, CheckCircle, FileText,
  DollarSign, Boxes, ClipboardList,
} from 'lucide-react';
...
export default function OfftakersDirectoryPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function fetchOfftakers() {
      const { data, error } = await supabase
        .from('service_providers')
        .select('*')
        .eq('provider_type', 'offtaker')
        .eq('is_listed', true)
  ...
  }, [providers, countryFilter, commodityFilter, search]);
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <div className="flex items-center gap-3 mb-3">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10">
                <Package className="w-6 h-6 text-[#5DB347]" />
              </div>
              <h1 className="text-2xl md:text-4xl font-bold">Offtakers</h1>
            </div>
            <p className="text-white/70 max-w-2xl">
              Connect with verified commodity buyers and offtakers across Africa. Secure contracts and reliable markets for your produce.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
            ...
            <h3 className="text-lg font-semibold text-gray-700 mb-2">No offtakers found</h3>
            <p className="text-gray-500 text-sm">Try adjusting your filters or check back later.</p>
          </motion.div>
        ) : (
          <>
            <p className="text-sm text-gray-500 mb-4">{filtered.length} offtaker{filtered.length !== 1 ? 's' : ''} found</p>
                  ...
                  >
                    <div className="p-6">
                      ...
                      {details.commodities && details.commodities.length > 0 && (
                        <div className="mb-3">
                          <p className="text-xs text-gray-400 mb-1.5 font-medium uppercase tracking-wide">Commodities Needed</p>
                          <div className="flex flex-wrap gap-1.5">
                            {(details.commodities as string[]).slice(0, 4).map((c) => (
                              <span key={c} className="px-2 py-0.5 bg-amber-50 text-amber-700 text-xs rounded-full font-medium">{c}</span>
                            ))}
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                        ...
                        {details.contract_type && (
                          <div className="flex items-center gap-1.5 text-gray-600">
                            ...
                            <span>{details.contract_type}</span>
                          </div>
                        )}
            ...
            >
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-lg font-bold text-[#1B2A4A]">Request Quote</h2>
                ...
                {submitSuccess ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="font-semibold text-[#1B2A4A] mb-1">Quote Request Sent</h3>
                    <p className="text-sm text-gray-500">The offtaker will review your request and respond shortly.</p>
                    <button
                      onClick={() => setModalOpen(false)}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g., Maize supply for Q3 2026"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Describe your supply offer or requirements..."
