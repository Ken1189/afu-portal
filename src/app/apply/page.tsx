      ...
      <>
        ...
        <section className="py-20 bg-white">
          <div className="max-w-2xl mx-auto px-4 text-center">
            <div className="bg-[#EBF7E5] rounded-2xl p-10">
              ...
              <Link href="/" className="inline-block bg-[#5DB347] hover:bg-[#449933] text-white px-8 py-3 rounded-lg font-semibold transition-colors">
                Back to Home
              </Link>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-4 mb-12">
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${step >= s ? "bg-[#5DB347] text-white" : "bg-gray-200 text-gray-400"}`}>
                  {s}
                </div>
                <span className={`text-sm font-medium ${step >= s ? "text-navy" : "text-gray-400"}`}>
                  {s === 1 ? "Select Tier" : "Your Details"}
                </span>
                {s < 2 && <div className={`w-16 h-0.5 ${step > 1 ? "bg-[#5DB347]" : "bg-gray-200"}`} />}
              </div>
            ))}
          </div>
            ...
            <div>
                  ...
                  <button
                    ...
                    className={`text-left rounded-2xl p-6 border-2 transition-all ${
                      selectedTier === key
                        ...
                        : "border-gray-200 hover:border-[#5DB347]/40"
                    }`}
                  >
              ...
              <div className="mt-8 text-center">
                <button
                  ...
                  className="bg-[#5DB347] hover:bg-[#449933] disabled:bg-gray-300 disabled:cursor-not-allowed text-white px-10 py-3 rounded-lg font-semibold transition-colors"
                >
                  Continue
                </button>
              </div>
          ...
          {step === 2 && (
            <div>
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setStep(1)} className="text-[#5DB347] hover:text-[#449933] text-sm font-medium">
                  ← Change Tier
                </button>
              </div>

              {selectedTier && (
                <div className="bg-[#EBF7E5] rounded-xl p-4 mb-8 flex justify-between items-center">
                  <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.firstName} onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-navy mb-2">Last Name *</label>
                    <input type="text" required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.lastName} onChange={(e) => setFormData({ ...formData, lastName: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-navy mb-2">Email *</label>
                    <input type="email" required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-navy mb-2">Phone</label>
                    <input type="tel" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-navy mb-2">Country *</label>
                    <select required className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.country} onChange={(e) => setFormData({ ...formData, country: e.target.value })}>
                  ...
                  <div>
                    ...
                    <input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" value={formData.organization} onChange={(e) => setFormData({ ...formData, organization: e.target.value })} />
                  </div>
                </div>
                ...
                {selectedTier !== "partner" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" placeholder="e.g., 5, 50, 500" value={formData.farmSize} onChange={(e) => setFormData({ ...formData, farmSize: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-navy mb-2">Main Crops</label>
                      <input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347]" placeholder="e.g., Blueberries, Cassava, Sesame" value={formData.crops} onChange={(e) => setFormData({ ...formData, crops: e.target.value })} />
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea required rows={4} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5DB347]/50 focus:border-[#5DB347] resize-none" placeholder={selectedTier === "partner" ? "Describe your organization and how you'd like to partner with AFU..." : "Describe your farming operation, current challenges, and what you're looking for from AFU..."} value={formData.about} onChange={(e) => setFormData({ ...formData, about: e.target.value })} />
                </div>
                ...
                <div className="flex gap-4">
                  ...
                  <button type="submit" className="bg-[#5DB347] hover:bg-[#449933] text-white px-10 py-3 rounded-lg font-semibold transition-colors">
                    Submit Application
                  </button>
                </div>
              </form>
