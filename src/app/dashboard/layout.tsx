...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/dashboard/programs", label: "Programmes", icon: "M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" },
    ],
  },
  {
    label: "Tools",
    ...
    links: [
      ...
      { href: "/dashboard/disputes", label: "Disputes", icon: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" },
    ],
  },
  {
    label: "Wallet & Assets",
...
function NavSection({
  ...
  return (
    <div>
      ...
      >
        <span className={hasActiveLink && collapsed ? "text-[#5DB347]" : ""}>{group.label}</span>
        <svg
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>(() => {
    ...
    navGroups.forEach((group) => {
      ...
      initial[group.label] = hasActive ? false : !group.defaultOpen;
    });
    return initial;
  });
  ...
  const renderSidebarNav = (onLinkClick?: () => void) => (
  ...
  const renderSidebarHeader = () => (
    <div className="gradient-navy p-5 text-white">
      <Link href="/" className="block mb-4">
        <Image src="/afu-logo.svg" alt="African Farming Union" width={120} height={40} className="h-10 w-auto object-contain rounded-md bg-white p-1" />
      </Link>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs opacity-70">Member &bull; AFU</p>
        </div>
      </div>
    </div>
  );

  const renderSidebarFooter = (onLinkClick?: () => void) => (
    <div className="p-3 border-t border-gray-100 space-y-0.5">
      <Link
        href="/farm"
        onClick={onLinkClick}
        className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-green-700 hover:bg-green-50 transition-colors"
      >
      ...
      {mobileNavOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute top-0 left-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col">
            <div className="gradient-navy p-5 text-white">
              <div className="flex items-center justify-between mb-4">
                <Link href="/" className="flex items-center" onClick={() => setMobileNavOpen(false)}>
                  <Image src="/afu-logo.svg" alt="African Farming Union" width={108} height={36} className="h-9 w-auto object-contain rounded-md bg-white p-1" />
                </Link>
                <button
                  onClick={() => setMobileNavOpen(false)}
            ...
            {renderSidebarFooter(() => setMobileNavOpen(false))}
          </aside>
        </div>
      )}
